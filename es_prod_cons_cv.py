...
class producerThread(threading.Thread):
    ...
    def run(self):
        ...
        with self.producer_cv:
            ...
            while not a_space_is_available(self.queue):
                logging.debug('Waiting')
...
def main():
    ...
    for i in range (N_CONSUMERS):
        ct = consumerThread(producer_cv, consumer_cv, queue, CONSUMER+str(i))
        ct.start()
        consumers.append(ct)
    ...
    for i in range (N_PRODUCERS):
        pt=producerThread(producer_cv,consumer_cv,queue,PRODUCER+str(i))
        pt.start()
        producers.append(pt)
